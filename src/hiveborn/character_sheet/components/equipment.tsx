...
import { equipmentTags, EquipmentTag } from "@/hiveborn/game_data/equipment_tags"
...
const EquipmentTagsDialog = ({ equipment, abilities }: { equipment: string; abilities: string }) => {
    ...
    const characterText = useMemo(
        () => ({
            abilities: normalizeForTagSearch(abilities),
            equipment: normalizeForTagSearch(equipment),
        }),
        [abilities, equipment],
    )
    ...
    const tagRows = useMemo(
        () =>
            equipmentTags.map((tag) => {
                const relevantSources = getRelevantSources(tag, characterText.equipment, characterText.abilities)
                return {
                    tag,
                    relevantSources,
    ...
    return (
        ...
        >
            <DialogHeader className="shrink-0">
                <DialogTitle>EQUIPMENT TAGS</DialogTitle>
            </DialogHeader>

            <div className="equipment-tag-search-wrap relative shrink-0">
                <Search className="equipment-tag-search-icon pointer-events-none absolute top-1/2 left-4 h-5 w-5 -translate-y-1/2 text-red-900/65" />
                <Input
                    ref={searchInputRef}
                    value={search}
                    ...
                    aria-label="Search equipment tags"
                />
            </div>

            <ScrollArea className="min-h-0 flex-1 pr-3">
                {filteredRows.length === 0 ? (
                    <div className="equipment-tag-empty rounded-md border border-dashed border-red-900/30 p-6 text-center text-sm text-muted-foreground">
                        No equipment tags match "{search}".
                    </div>
                ) : (
                    <div className="grid gap-2 pb-1">
                        {filteredRows.map(({ tag, relevantSources, isRelevant }, index) => (
                            <article
                                key={tag.name}
                                className={cn(
                                    "equipment-tag-card rounded-md border p-3 text-left",
                                    isRelevant
...
const getRelevantSources = (tag: EquipmentTag, equipment: string, abilities: string) => {
    const tagRegex = getTagRegex(tag)
    const relevantSources = []
    if (tagRegex.test(equipment)) relevantSources.push("Equipment")
    if (tagRegex.test(abilities)) relevantSources.push("Abilities")
...
const getTagRegex = (tag: EquipmentTag) => {
